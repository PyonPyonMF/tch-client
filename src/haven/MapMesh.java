...
public class MapMesh implements RenderTree.Node, Disposable {
	...
	public MapSurface() {
		...
		for(int x = vs.ul.x; x < vs.br.x; x++) {
		    surf[vs.o(x, y)] = new Vertex(x * (float)tilesz.x, y * -(float)tilesz.y, CFG.FLATWORLD.get() ? 0 : (float) map.getfz(ul.add(x, y)));
		}
	    }
	...
	float cz = CFG.FLATWORLD.get() ? 0 : (float)map.getcz(cc);
	Coord ult, brt;
	{
